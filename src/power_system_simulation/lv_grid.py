...
class LVGrid:
    ...
    def simulate_ev_penetration(
        ...
        evs_per_feeder = int(np.floor(penetration_level * total_houses / number_of_feeders))

        total_evs_needed = evs_per_feeder * number_of_feeders

        if total_evs_needed > self.ev_profile.shape[1]:
            raise ValueError(
                f"{total_evs_needed} EV profiles are needed, but only {self.ev_profile.shape[1]} are available."
            )

        # Map sym_load ID -> node ID
        sym_loads = self.input_data[ComponentType.sym_load]
        load_node_by_id = {
            int(sym_loads[AttributeType.id][i]): int(sym_loads[AttributeType.node][i]) for i in range(len(sym_loads))
        }

        # Map load ID -> profile column name
        # This is useful because parquet columns can sometimes be strings.
        active_column_by_load_id = {int(column): column for column in active_with_ev.columns}
        ...
        if total_evs_needed == 0:
            chosen_ev_profiles = np.array([], dtype=object)
        else:
        ...
        for feeder_id in self.feeder_ids:
            downstream_nodes = set(self.graph.find_downstream_vertices(int(feeder_id)))

            feeder_load_ids = [
                load_id
                for load_id, node_id in load_node_by_id.items()
                if node_id in downstream_nodes and load_id in active_column_by_load_id
            ]

            if len(feeder_load_ids) < evs_per_feeder:
                raise ValueError(
                    f"Feeder {feeder_id} has only {len(feeder_load_ids)} houses, but {evs_per_feeder} EVs are required."
                )
            ...
            for load_id in selected_load_ids:
                ...
                active_with_ev[active_column] = active_with_ev[active_column] + self.ev_profile[ev_profile_column]
        ...
        line_ids_arr = self.input_data[ComponentType.line][AttributeType.id]
        ...
        if isinstance(active_with_ev.index, pd.DatetimeIndex) and len(active_with_ev.index) > 1:
            dt_hours = (active_with_ev.index[1] - active_with_ev.index[0]).total_seconds() / 3600
        else:
            dt_hours = 1.0
    ...
    def optimize_tap_position(self, criteria: OptimizationCriteria = OptimizationCriteria.MIN_VOLTAGE_DEVIATION) -> int:
        ...
        # swap if wrong order
        tap_lower = min(tap_min, tap_max)
        tap_upper = max(tap_min, tap_max)
